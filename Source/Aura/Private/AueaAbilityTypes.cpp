...
bool FAueaGameplayEffectContext::NetSerialize(FArchive& Ar, UPackageMap* Map, bool& bOutSuccess)
{
	uint32 RepBits = 0;
	if (Ar.IsSaving())
	{
		...
		if (DebuffFrequency > 0.f) RepBits |= 1 << 11;
		if (DebuffDuration > 0.f) RepBits |= 1 << 12;
	...
	if (RepBits & (1 << 12)) Ar << DebuffDuration;
	if (RepBits & (1 << 13))
	{
		if (Ar.IsLoading())
		{
